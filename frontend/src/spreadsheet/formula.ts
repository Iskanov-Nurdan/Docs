 ...
 */
/**
 * Потолок строк на листе.
 *
 * Прежние пять тысяч упирались в живой журнал: за год рейсов набирается
 * заметно больше. Сетка рисует только видимое, а смещения строк считаются
 * умножением, пока не включён фильтр, поэтому размер листа сам по себе
 * ничего не замедляет — тяжелеют только полные обходы: пересчёт фильтра
 * и сбор уникальных значений столбца.
 */
